...
% number_of_nodes_vector = 20:1:100;
number_of_nodes_vector = 3;
values_of_nodes = [1; 2; 1];
number_of_value_levels = 2;
network_structure_index = 1;
...
%%
counter = 1;
Correct_votes_of_runs = cell(length(number_of_nodes_vector), 1);
Draw_votes_of_runs = cell(length(number_of_nodes_vector), 1);
Correct_votes_std_of_runs = cell(length(number_of_nodes_vector), 1);
initial_values_of_nodes = cell(length(number_of_nodes_vector), 1);
number_of_phases = cell(length(number_of_nodes_vector), 1);
number_of_rounds = cell(length(number_of_nodes_vector), 1);
number_of_time_slots_in_corrosion = cell(length(number_of_nodes_vector), 1);
number_of_periods = cell(length(number_of_nodes_vector), 1);
number_of_time_slots_in_checkTermination = cell(length(number_of_nodes_vector), 1);
number_of_time_slots_in_phase = cell(length(number_of_nodes_vector), 1);
total_number_of_time_slots = cell(length(number_of_nodes_vector), 1);
total_number_of_time_slots_average = cell(length(number_of_nodes_vector), 1);
votes = cell(length(number_of_nodes_vector), 1);
run = 0;
for number_of_nodes = number_of_nodes_vector
    run = run + 1;
    str = sprintf('run %d out of %d runs', run, length(number_of_nodes_vector));
    disp(str);
...
%         value = assign_initial_values_to_nodes(number_of_nodes, number_of_value_levels, delta);
    ...
    disp('The correctness rate for this run was:')
    disp(correctness_rate(run));
    disp('The draw rate for this run was:')
    disp(draw_rate(run));
...
%% plot result:
f1 = figure;
% stem(number_of_nodes_vector, correctness_rate,'filled')
xlabel('Number of nodes')
ylabel('Percentage of correct votes')
ylim([0 110])
hold on
plot(number_of_nodes_vector, correctness_rate, '-o', 'MarkerFaceColor', 'r')
%set(gca, 'XTick', round(number_of_nodes_vector,2));
%xlim([min(number_of_nodes_vector): 10 : max(number_of_nodes_vector)]);
grid on

f2 = figure;
% stem(number_of_nodes_vector, draw_rate,'filled')
xlabel('Number of nodes')
ylabel('Percentage of draw votes')
ylim([0 110])
hold on
plot(number_of_nodes_vector, draw_rate, '-o', 'MarkerFaceColor', 'r')
%set(gca, 'XTick', round(number_of_nodes_vector,2));
%xlim([min(number_of_nodes_vector),max(number_of_nodes_vector)]);
...
save number_of_nodes_vector.mat number_of_nodes_vector
